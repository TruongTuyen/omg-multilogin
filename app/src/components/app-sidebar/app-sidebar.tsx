				...
				<div className="flex column side-menu">
					<div className="side-quick-actions">
						...
						<div className="side-actions">
							<a className="action-create-new" href="#/new-profile">Create new</a>
						</div>
					</div>
	...
	renderMenuItem = ({ key, icon, title, href }: SideMenuItem): JSX.Element => {
		...
		return (
			<li key={key} className={`side-menu-list`}>
				<a
					className={`side-menu-item fs-22 ri-${icon}-${isActive ? 'fill' : 'line'}`}
					style={{ color: isActive ? '#fff' : '' }}
					href={href}
				><span className={"menu-item-name"}>{title}</span></a>
			</li>
		)
	}
